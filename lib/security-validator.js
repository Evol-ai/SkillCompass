  ...
  checkUnconfirmedExternalCalls(content) {
    const externalCallPatterns = [
      ...
      },
      {
        ...
        severity: "medium" // Tool declarations handled and confirmed by host model
      }
    ];

    externalCallPatterns.forEach(({ pattern, description, severity }) => {
      let match;
      while ((match = pattern.exec(content)) !== null) {
        const line = this.getLineNumber(content, match.index);
  ...
  calculateDeductions() {
    ...
    this.findings.forEach(finding => {
      switch (finding.severity) {
        ...
        case 'medium': deduction += 1.5; break;
