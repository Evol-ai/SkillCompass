...
class StructureValidator {
  ...
  validate(filePath) {
    ...
    // Calculate score
    const frontmatterScore = this.calculateSubScore(this.checks.frontmatter);
    const formatScore = this.calculateSubScore(this.checks.format);
    const declarationsScore = this.calculateSubScore(this.checks.declarations);
  ...
  validateDeclarations(frontmatter, bodyContent) {
    ...
    const hasTriggerMechanism = 
      ...
      frontmatter.hooks || 
      frontmatter.globs || 
      (frontmatter.description && frontmatter.description.length > 10);
  ...
  validateScopeBoundaries(frontmatter, bodyContent) {
    const hasNotFor = 
      (frontmatter.description && frontmatter.description.toLowerCase().includes('not for')) ||
      bodyContent.toLowerCase().includes('not for') ||
      bodyContent.toLowerCase().includes('not intended for');

    const hasClearPurpose = frontmatter.description && frontmatter.description.length > 30;
  ...
  parseYAML(yamlContent) {
    ...
    lines.forEach(line => {
      ...
      if (inMultiline) {
        if (line.startsWith(' '.repeat(indentLevel + 2))) {
      ...
      if (match) {
        ...
        if (value.trim() === '' || value.trim() === '>' || value.trim() === '|') {
          ...
          try {
            ...
            result[cleanKey] = [value.slice(1, -1).trim()];
          }
        } else if (value.includes(',') && !value.includes('"')) {
          // Simple array
          result[cleanKey] = value.split(',').map(v => v.trim());
        } else {
          // Simple value
