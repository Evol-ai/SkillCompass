  ...
  static RULES = [
    ...
    // R3 is user-initiated, not engine-evaluated
    {
      ...
      reason(s) {
        ...
        return `Uses ${sizeKB}KB context, only ${s.use_count_14d} invocation(s) in 14 days`;
      },
      evidence(s) {
    ...
    },
    {
      ...
      reason(s) {
        const sizeKB = (s.total_size / 1024).toFixed(1);
        return `Uses ${sizeKB}KB context but never invoked, consider cleanup or evaluation`;
      },
      evidence(s) {
