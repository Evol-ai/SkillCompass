...
const RULES = [
  ...
  },
  {
    ...
    category: 'hygiene',
    priority: 'P3',
    reason: 'Skill installed >7 days ago but has never been evaluated.',
    check(signals) {
      const { first_seen_at, last_eval_at, last_improve_at, version_count, file_modified_at } = signals;
      if (!first_seen_at) return false;
      const installedDays = daysSince(first_seen_at);
      const now = Date.now();
      const dayMs = 86_400_000;
      // R2: also skip if file was recently modified (user is actively using/editing)
      const fileRecentlyModified = file_modified_at &&
        (now - new Date(file_modified_at).getTime()) / dayMs < 7;
      if (installedDays > 7 && !last_eval_at && !last_improve_at &&
          version_count === 0 && !fileRecentlyModified) {
        return true;
      }
      return false;
    },
    evidence(signals) {
      return [
        ...
        `file_modified_at: ${signals.file_modified_at ?? 'never'}`,
...
class InboxEngine {
  ...
  collectSignals(skillName, setupEntry) {
    ...
    const file_modified_at = setupEntry.modified_at || null;
  ...
  runDigest(skillEntries) {
    ...
    for (const entry of skillEntries) {
      ...
      this.store.updateSkillCache(skillName, {
        ...
        file_modified_at:  signals.file_modified_at,
        duplicate_of:      signals.duplicate_of
      });
    }
